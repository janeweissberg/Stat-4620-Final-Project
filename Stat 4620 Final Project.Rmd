...
## PART 3: check modeling assumptions
...
lm_model <- lm(ViolentCrimesPerPop ~ PctPopUnderPov + PctNotHSGrad +
                 PctUnemployed + PctKids2Par + PctFam2Par +
                 medIncome + perCapInc,
               data = train)
...
hist(model_data$Residuals, breaks = 30, col = "green",
     main = "Histogram of Residuals", xlab = "Residuals")

# 2. Q–Q plot 
qqnorm(model_data$Residuals)
qqline(model_data$Residuals, col = "red")

# 3. Residuals vs Fitted 
plot(model_data$Fitted, model_data$Residuals,
     col = "orange", pch = 16,
...
predictors <- train %>%
  select(PctPopUnderPov, PctNotHSGrad, PctUnemployed,
         PctKids2Par, PctFam2Par, medIncome, perCapInc)
cor(predictors, use = "pairwise.complete.obs")


```

Explanation for Model Assumptions Check:

The histogram and Q–Q plot of residuals show that the residuals are roughly symmetric and centered around zero, suggesting they are approximately normally distributed.
The residuals vs fitted plot indicates no major curvature, but the spread increases slightly with higher fitted values, meaning variance may not be perfectly constant.
The correlation matrix shows some strong correlations (between medIncome and perCapInc), suggesting moderate multicollinearity. Overall, while a few assumptions are not
perfectly met, the model still provides a reasonable fit for explaining how socioeconomic factors relate to violent crime rates.


## PART 4: Ridge Regression Model Validation
...
#  Prepare data
formula <- as.formula("ViolentCrimesPerPop ~ .")
...
# Align columns between training and test matrices
...
y_pred <- predict(lasso_mod, newx = x_test, s = "lambda.min")
...
# Print results
cat("Model Validation Results (LASSO Regression):\n")
...
plot(y_test, y_pred,
     main = "Predicted vs Actual Violent Crime Rates (LASSO)",
     xlab = "Actual Values",
...
Explanation of Model Validation (LASSO Regression):

The LASSO model performs very well, with an $R^2$ of 0.999, meaning it explains almost all the variation in violent crime rates.
The RMSE (19.48) and MAE (15.54) are small, showing that prediction errors are low.
The plot of predicted vs actual values shows points closely following the red line, confirming that the model’s predictions are highly accurate and consistent.
